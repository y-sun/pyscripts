...
for ifl in fls:
    ...
    if(args.match):
        for k in range(1,data.shape[1]):
            if(k==1):
                plt.plot(xm,data[:,k],linestyle=lshp[ct],color=colors[ct],label=names[ct])
            else:
                plt.plot(xm,data[:,k],linestyle=lshp[ct],color=colors[ct])
    else:
        for k in range(1,data.shape[1]):
            if(k==1):
                plt.plot(data[:,0],data[:,k],linestyle=lshp[ct],color=colors[ct],label=names[ct])
            else:
                plt.plot(data[:,0],data[:,k],linestyle=lshp[ct],color=colors[ct])
    ct+=1
...
if(args.ylim is not None):
    ...
    plt.ylim(ymin,ymax)
plt.tight_layout()
plt.savefig("band-structure.png")
plt.show()
